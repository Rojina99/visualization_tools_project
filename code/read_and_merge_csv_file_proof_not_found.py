"""Script to get plots for proof not found data set i.e. Time Out dataset
We donot take two other features Clause Processing Time Out and nan because they have zero as values
 Here merged 3 subplots is min, mean and max of features
and merged 5 subplots subplots with min max mean of its features.
...
# merged_data_frame_unique = merged_data_frame.drop(columns=['Tautologies Deleted'])

grouped_min = merged_data_frame.select_dtypes(include=[np.number]).min()
grouped_mean = merged_data_frame.select_dtypes(include=[np.number]).mean().astype(int)
grouped_max = merged_data_frame.select_dtypes(include=[np.number]).max()
...
# Show the plot
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "proof_not_found_merged_3_subplots.pdf"))
# plt.show()
plt.close()

num_features = len(merged_data_frame.columns[3:-1])
...
for i, col in enumerate(merged_data_frame.columns[3:-1]):
    ax = axes[i]
...
# ax = axes[-1]
# ax.bar(merged_data_frame.columns[3:-1], grouped_min, color='blue')
...
# plt.show()
plt.savefig(os.path.join(OUTPUT_DIR, "proof_not_found_merged_5_subplots.pdf"))
plt.close()
...
grouped_min_log = np.log1p(merged_data_frame.select_dtypes(include=[np.number]).min())
grouped_mean_log = np.log1p(merged_data_frame.select_dtypes(include=[np.number]).mean().astype(int))
grouped_max_log = np.log1p(merged_data_frame.select_dtypes(include=[np.number]).max())
...
# Show the plot
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "proof_not_found_merged_3_log_subplots.pdf"))
# plt.show()
plt.close()
...
# plt.show()
plt.savefig(os.path.join(OUTPUT_DIR, "proof_not_found_merged_5_log_subplots.pdf"))
plt.close()
