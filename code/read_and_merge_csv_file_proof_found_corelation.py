"""Script to get scatter plot and correlation for proof found data set against its feature.
The plot name are self explanatory
Result is saved on result/proof_found/correlation directory"""
import os
import pdb
...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'result/proof_found/correlation')
...
for ax, column, title in zip(axs.flatten(), comparison_columns, titles):
    ...
    # Add regression line
    sns.regplot(x='Initial Clause Count', y=column, data=merged_data_frame_unique, scatter=False, ci=None, color='blue', ax=ax)
...
# plt.show()
# Show the plot
plt.savefig(os.path.join(OUTPUT_DIR, "proof_found_corr_icc.pdf"))
plt.close()
...
for ax, column, title in zip(axs.flatten(), comparison_columns, titles):
    ...
    # Add regression line
    sns.regplot(x='Processed Clause Count', y=column, data=merged_data_frame_unique, scatter=False, ci=None, color='blue', ax=ax)
    # Set title with correlation coefficient
    ax.set_title(f'{title} (r={correlation:.2f})')
    # Optional: Improve tick label readability
    ax.tick_params(axis='x', labelrotation=45)

plt.savefig(os.path.join(OUTPUT_DIR, "proof_found_corr_pcc.pdf"))
plt.close()
...
plt.savefig(os.path.join(OUTPUT_DIR, "proof_found_corr_ucc.pdf"))
plt.close()
...
plt.savefig(os.path.join(OUTPUT_DIR, "proof_found_corr_fc.pdf"))
plt.close()
...
# plt.show()
# Show the plot
plt.savefig(os.path.join(OUTPUT_DIR, "proof_found_corr_rc.pdf"))
plt.close()